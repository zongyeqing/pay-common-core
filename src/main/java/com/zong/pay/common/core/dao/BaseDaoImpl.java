...
import com.zong.pay.common.core.mybatis.interceptor.ExecutorInterceptor;
...
public class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
    ...
    public long insert(List<T> list) {
        if(list == null || list.size() <= 0)
            return 0;
        int result = sessionTemplate.insert(getStatement(SQL_INSERT), list);
